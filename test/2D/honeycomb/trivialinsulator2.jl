...
filledfock = FockSpace(m for (m, v) in localspectrum|>geteigenvalues if v < 0.04)
emptyfock = FockSpace(m for (m, v) in localspectrum|>geteigenvalues if v > (1-0.04))
...
filledseeds = filledseeds + siteBseed 
emptyseeds = emptyseeds + siteAseed 
