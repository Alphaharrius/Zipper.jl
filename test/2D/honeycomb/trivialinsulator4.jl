...
filledfock = FockSpace(m for (m,v) in sortedevalpair[1:24])
emptyfock = FockSpace(m for (m,v) in sortedevalpair[end-23:end])
...
centerfilledseeds = getregionstates(localcorrelations=filledlocalcorrelations, grouping=[12])[1]
...
centeremptyseeds = getregionstates(localcorrelations=emptylocalcorrelations, grouping=[12])[1]
...
siteAfilledseed = getregionstates(localcorrelations=siteAlocalfilledcorrelations, grouping=[4])[1]
siteAemptyseed = getregionstates(localcorrelations=siteAlocalemptycorrelations, grouping=[4])[1]
...
siteBfilledseed = getregionstates(localcorrelations=siteBlocalfilledcorrelations , grouping=[4])[1]
siteBemptyseed = getregionstates(localcorrelations=siteBlocalemptycorrelations, grouping=[4])[1]
# siteBfilledseed = c3 * siteBfilledseed
# siteBemptyseed = c3 * siteBemptyseed


filledseeds = centerfilledseeds+siteAfilledseed
emptyseeds = centeremptyseeds+siteBemptyseed
...
0.0021896859670931293

(wannieremptystates|>RegionState)[3]|>visualize

(wannierfilledstates|>RegionState)[11]|>visualize

0.0021896859670926705, 0.0021896859670929077, [24,24]