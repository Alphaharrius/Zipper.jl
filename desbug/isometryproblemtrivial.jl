...
[(localcorrelations[mode,mode]|>rep)[1,1]|>abs for mode in firsthexagonalregionfock]
# potentialfilledseeds = for mode in 
...
eerankedmodes = sort([(mode,ee) for (mode,ee) in firsteecontour],by=x->x[2])
eerankedmodes[1:6]
offsetofmodes(Subset([mode for (mode,ee) in eerankedmodes][1:6]))|>visualize