...
correlations = initblock * correlations * initblock'
...
truncate = fourier(stripfrozencorrelations|>getoutspace, truncationregionfock, homemappings) / sqrt(stripfrozencorrelations|>getoutspace|>getcrystal|>vol)
...
truncationregionindices = Iterators.product(truncatedregioncorrelations|>getoutspace, truncatedregioncorrelations|>getoutspace)
...
visualize(region1localstates, markersizemultiplier=20, markersizescaling=0.3)
...
visualize(region2localstates, markersizemultiplier=20, markersizescaling=0.3)
...
wannierfilledisometry = wannierprojection(crystalisometries=stripfilledstates|>geteigenvectors, crystal=scaledcrystal, crystalseeds=crystalseeds)
wanniercrystal = wannierfilledisometry|>getinspace|>getcrystal
rightrestrict = fourier(wannierfilledisometry|>getinspace, wannierfilledisometry|>getinspace|>unitcellfock|>RegionFock) / (wanniercrystal|>vol|>sqrt)
...
remapper = spatialremapper(wannierlocalisometry|>getoutspace, getsphericalregion(crystal=blockedcrystal, radius=2, metricspace=blockedcrystal|>getspace))
wannierlocalisometry = remapper * wannierlocalisometry
wanniermetalprojector = crystalprojector(localisometry=wannierlocalisometry, crystalfock=blockedcrystalfock)
...
function getcourierseeds(region::Region)
    regionfock = quantize(region, 1)
    ...
    return getregionstates(localcorrelations=localcorrelations, grouping=[1])[1]
end
