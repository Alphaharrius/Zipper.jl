...
scale = Scale([12 0; 0 12], crystalfock|>getcrystal|>getspace)
...
localregion = getsphericalregionwifcenter(center=localcenter1input,crystal=blockedcrystal, radius=1/3-0.02, metricspace=blockedspace|>orthospace)
localseedingfock = quantize(localregion,1)
visualize(localregion)
modebydist = groupmodesbydist(region = localregion,regionfock = localseedingfock,center = localcenter1input) 
modebydist[6]
...
function firststepgmera(correlations::FockMap,localcenter1,localcenter2,localcenter3,localcenter4,localcenter5)
    ...
    function fulllocalwannierization(correlations::FockMap,localcenter::Offset,radius::Real=1/3-0.02,selectionstragedy::Function=modeselectionbycount(12))
        ...
        frozenseeds = idmap(localseedingfock, localseedingfock)[:,FockSpace(mode[2] for mode in modebydist[10])+FockSpace(mode[2] for mode in modebydist[9])+FockSpace(mode[2] for mode in modebydist[8])] 
        ...
        courierseeds = idmap(localseedingfock, localseedingfock)[:,FockSpace(mode[2] for mode in modebydist[1])+FockSpace(mode[2] for mode in modebydist[2])+FockSpace(mode[2] for mode in modebydist[3])+FockSpace(mode[2] for mode in modebydist[4])+FockSpace(mode[2] for mode in modebydist[5])+FockSpace(mode[2] for mode in modebydist[6])+FockSpace(mode[2] for mode in modebydist[7])]
    ...
    wannierizedfrozens4,wannierizedcouriers4,frozenseeds4,courierseeds4 = fulllocalwannierization(correlations, localcenter4)
    wannierizedfrozens5,wannierizedcouriers5,frozenseeds5,courierseeds5 = fulllocalwannierization(correlations, localcenter5)
...
# visualize(localregion)
# modebydist = groupmodesbydist(region = localregion,regionfock = localseedingfock,center = localcenter1input) 
# localiso = localisometries(blockedcorrelations, localseedingfock, selectionstrategy=modeselectionbycount(12))
# frozenseeds = idmap(localseedingfock, localseedingfock)[:,FockSpace(mode[2] for mode in modebydist[10])+FockSpace(mode[2] for mode in modebydist[9])+FockSpace(mode[2] for mode in modebydist[8])] 
# courierseeds = idmap(localseedingfock, localseedingfock)[:,FockSpace(mode[2] for mode in modebydist[1])+FockSpace(mode[2] for mode in modebydist[2])+FockSpace(mode[2] for mode in modebydist[3])+FockSpace(mode[2] for mode in modebydist[4])+FockSpace(mode[2] for mode in modebydist[5])+FockSpace(mode[2] for mode in modebydist[6])+FockSpace(mode[2] for mode in modebydist[7])]
# wannierfrozens = _localwannierization(localiso[:filled]+localiso[:empty], frozenseeds)
# wanniercouriers = _localwannierization(localiso[:courier], courierseeds)