...
filledfock = FockSpace(m for (m, v) in localspectrum|>geteigenvalues if v < 0.000002)
emptyfock = FockSpace(m for (m, v) in localspectrum|>geteigenvalues if v > (1-0.000003))
...
siteAcourierseed = getregionstates(localcorrelations=siteAlocalcouriercorrelations, grouping=[4])[1]
siteBcourierseed = getregionstates(localcorrelations=siteBlocalcouriercorrelations, grouping=[4])[1]
...
couriercorrelations|>getinspace