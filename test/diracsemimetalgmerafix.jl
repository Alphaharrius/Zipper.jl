...
firstgmerarea[:couriercorrelations]
...
(refcorrelations.inspace |> getcrystal).unitcell
visualize((refcorrelations.inspace |> getcrystal).unitcell)
# trialsamplepoints::Subset{Offset} = refcrystalpoints + c6^3 * refcrystalpoints
refblockedmodes::Subset{Mode} = quantize(:pos, (refcorrelations.inspace |> getcrystal).unitcell, 1)
visualize(regioncorrelations(refcorrelations, FockSpace{Region}(refblockedmodes)) |> eigspec)

H = energyspectrum |> FockMap
scaler*blocker*H*blocker'*scaler'
firstgmerarea[:courierisometry]
visualize(scaler*blocker*H*blocker'*scaler'|>crystalspectrum)