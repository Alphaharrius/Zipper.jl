...
filledfock = FockSpace(m for (m, v) in localspectrum|>geteigenvalues if v < 0.003)
emptyfock = FockSpace(m for (m, v) in localspectrum|>geteigenvalues if v > (1-0.003))
...
centerfilledseeds = getregionstates(localcorrelations=filledlocalcorrelations, grouping=[3])[1]
...
centeremptyseeds = getregionstates(localcorrelations=emptylocalcorrelations, grouping=[3])[1]
...
centerfilledseeds = c6*centerfilledseeds
centeremptyseeds = c6*centeremptyseeds

@info "Wannierizing filled bands..."
crystalfilledrseeds = crystalisometries(localisometry=centerfilledseeds|>FockMap, crystalfock=blockedcrystalfock, addinspacemomentuminfo=true)
...
# siteAcourierseed = getregionstates(localcorrelations=siteAlocalcouriercorrelations, grouping=[1])[1]
# siteBcourierseed = getregionstates(localcorrelations=siteBlocalcouriercorrelations, grouping=[1])[1]
