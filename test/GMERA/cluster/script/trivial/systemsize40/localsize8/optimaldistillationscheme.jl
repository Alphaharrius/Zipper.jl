...
[onsitepotential for onsitepotential in reverse([onsite for onsite in range(-0.9,-0.5,5)])]

for onsitepotential in reverse([onsite for onsite in range(-0.7,-0.7,1)])
# onsitepotential = -0.2
    fiodir("/Users/slwongag/Desktop/data/trivial/systemsize40/localsize8/optimaldistillationscheme[72,36,0]/onsitepotential_$onsitepotential")
    systemsize=40
    ...
    gmera1firststepdata = gmerafirststepbycount(blockedcorrelations,72)
    gmera1secondstepdata = gmerasecondstepbycount(gmera1firststepdata[:couriercorrelations],36,12)
    gmera1thirdstepdata = gmerathirdstepbycount(gmera1secondstepdata[:couriercorrelations],0,6)
